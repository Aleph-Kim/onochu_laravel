...
<div class="p-5 md:py-10 md:px-[30px]">
    <div class="flex items-center mb-[60px] flex-wrap max-lg:flex-col max-lg:text-center max-lg:gap-5">
        <img src="{{ $albumInfo['img_url'] }}?/dims/resize/500x500/quality/90" class="w-[250px] h-[250px] rounded-2xl object-cover mr-5 max-lg:mr-0 max-sm:w-[200px] max-sm:h-[200px] shadow-[0_8px_24px_rgba(0,0,20,0.12)]">
        <div>
            <div class="text-[2.5em] font-bold tracking-tight text-[#111] max-sm:text-[2em]">{{ $albumInfo['title'] }}</div>
    ...
    <div class="text-2xl font-bold tracking-tight mb-6 text-[#111]">수록곡</div>
    <div class="ml-[30px] max-sm:ml-0">
        @foreach($songsInfo as $song)
            <div class="song-box group flex justify-between h-[110px] w-full p-5 mb-2 rounded-2xl bg-white border border-[#ebebf0] transition-all hover:shadow-md hover:border-[#d8d8e8] cursor-pointer">
                <div class="flex flex-1 min-w-0 overflow-hidden">
                    <span class="w-[70px] mr-[10px] flex-shrink-0 max-sm:mr-0">
                        <img src="{{ $albumInfo['img_url'] }}?/dims/resize/350x350/quality/90" alt="" class="h-full rounded-xl">
                    </span>
                    <div class="max-w-[75%] px-[10px] flex flex-col justify-around">
                        <div class="text-primary overflow-hidden text-ellipsis whitespace-nowrap font-medium">
                            @if($song['song']['title_yn'] == 'Y')
                                <span class="text-[0.7em] font-bold text-primary bg-[#f0f0fa] border border-primary/20 py-[2px] px-[6px] mr-[6px] inline-block rounded-full tracking-wide">title</span>
                            @endif
                            {{ $song['song']['title'] }}
                        </div>
                ...
                <div class="icon-box hidden items-center flex-shrink-0 gap-[5px] group-hover:flex">
                    <a href="{{ route('recommends.create', ['id' => $song['song']['flo_id']]) }}" class="flex p-[7px] fill-[#8b8b9a] cursor-pointer rounded-full transition-all hover:fill-[#5b5bd6] hover:bg-[#f0f0fa]">
                    ...
                    <a href="{{ route('song.detail', ['id' => $song['song']['flo_id']]) }}" class="flex p-[7px] fill-[#8b8b9a] cursor-pointer rounded-full transition-all hover:fill-[#5b5bd6] hover:bg-[#f0f0fa]">
                    ...
                    <a href="{{ route('artist.detail', ['id' => $song['artists'][0]['flo_id']]) }}" class="flex p-[7px] fill-[#8b8b9a] cursor-pointer rounded-full transition-all hover:fill-[#5b5bd6] hover:bg-[#f0f0fa]">
