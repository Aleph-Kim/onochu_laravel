...
@section('content')
    <div class="p-5 md:py-10 md:px-[30px]">
        <div class="flex items-center mb-[60px] flex-wrap max-lg:flex-col max-lg:text-center max-lg:gap-5">
            <img src="{{ $albumInfo['img_url'] }}?/dims/resize/500x500/quality/90"
                 class="w-[250px] h-[250px] rounded-2xl object-cover mr-5 max-lg:mr-0 max-sm:w-[200px] max-sm:h-[200px] shadow-[0_8px_24px_rgba(0,0,20,0.12)]">
            <div>
                <div
                    class="text-[2.5em] font-bold tracking-tight text-[#111] max-sm:text-[2em]">{{ $albumInfo['title'] }}</div>
            ...
            @foreach($songsInfo as $song)
                <div
                        ...
                        <div class="max-w-[75%] px-[10px] flex flex-col justify-around">
                            <div class="text-primary overflow-hidden text-ellipsis whitespace-nowrap font-medium">
                                @if($song['song']['title_yn'] == 'Y')
                                    <span
                                        class="text-[0.7em] font-bold text-primary bg-[#f0f0fa] border border-primary/20 py-[2px] px-[6px] mr-[6px] inline-block rounded-full tracking-wide">title</span>
                                @endif
                                {{ $song['song']['title'] }}
                            </div>
                    ...
                    <div class="icon-box hidden items-center flex-shrink-0 gap-[5px] group-hover:flex">
                        ...
                        <a href="{{ route('song.detail', ['id' => $song['song']['flo_id']]) }}"
                           class="flex p-[7px] fill-[#8b8b9a] cursor-pointer rounded-full transition-all hover:fill-[#5b5bd6] hover:bg-[#f0f0fa]">
                            <svg viewBox="0 0 24 24" class="h-[35px]">
                                <g>
                                    <path
                        ...
                        <a href="{{ route('artist.detail', ['id' => $song['artists'][0]['flo_id']]) }}"
                           class="flex p-[7px] fill-[#8b8b9a] cursor-pointer rounded-full transition-all hover:fill-[#5b5bd6] hover:bg-[#f0f0fa]">
                            <svg viewBox="0 0 24 24" class="h-[35px]">
                                <g>
                                    <path
                                        d="M22,10h-4v2v3.51C17.58,15.19,17.07,15,16.5,15c-1.38,0-2.5,1.12-2.5,2.5c0,1.38,1.12,2.5,2.5,2.5 c1.36,0,2.46-1.08,2.5-2.43V12h3V10z M3.06,19c0.38-3.11,2.61-6.1,7.94-6.1c0.62,0,1.19,0.05,1.73,0.13l0.84-0.84 c-0.58-0.13-1.19-0.23-1.85-0.26C13.58,11.59,15,9.96,15,8c0-2.21-1.79-4-4-4C8.79,4,7,5.79,7,8c0,1.96,1.42,3.59,3.28,3.93 C4.77,12.21,2,15.76,2,20h10.02L12,19H3.06z M8,8c0-1.65,1.35-3,3-3s3,1.35,3,3s-1.35,3-3,3S8,9.65,8,8z"
                                        class="style-scope yt-icon"></path>
                                </g>
                            </svg>
                        </a>
