...
<header class="w-full bg-white/80 backdrop-blur-xl border-b border-[#ebebf0] sticky top-0 z-[999]">
    <div
        class="mx-auto px-[10px] sm:px-[20px] md:px-[30px] lg:px-[50px] xl:px-[200px] flex items-center justify-between h-[70px]">
        ...
        <div id="searchFormWrap" class="flex items-center justify-center">
            ...
            <form id="searchForm" class="relative w-[300px] mx-auto max-sm:w-4/5 max-sm:mx-0 max-sm:hidden"
                  action="{{ route('search') }}">
                ...
                <button
                    class="w-[38px] p-[7px] absolute right-0 top-1/2 -translate-y-1/2 cursor-pointer flex items-center">
                    <svg viewBox="0 0 24 24" class="w-full fill-[#b0b0c0]">
                        <g>
                            <path
                ...
                @if(session('user'))
                    @if(request()->is('mypage'))
                        <a href="{{ route('logout') }}" type="button"
                           class="px-4 py-1.5 text-[#555] text-sm font-medium cursor-pointer hover:text-primary transition-colors duration-150">로그아웃</a>
                    @else
...
<div
    class="loader-container fixed top-0 left-0 w-screen h-screen hidden items-center justify-center bg-black/40 backdrop-blur-sm">
    <div
